...
class BatchOptimizationRunner:
    ...
    def __init__(self, methods=['bayesian', 'hyperband', 'cmaes'],
    ...
    def generate_comparison_report(self):
        ...
        with open(report_file, 'w') as f:
            ...
            for method in sorted(df['method'].unique()):
                ...
                f.write(f"  Average Validation F1: {method_df['best_val_f1'].mean():.4f} ± {method_df['best_val_f1'].std():.4f}\n")
                
                if 'test_f1' in method_df.columns and method_df['test_f1'].notna().any():
                    f.write(f"  Average Test F1: {method_df['test_f1'].mean():.4f} ± {method_df['test_f1'].std():.4f}\n")
            ...
            f.write(f"   Average F1: {method_avg.loc[best_method, 'mean']:.4f} ± {method_avg.loc[best_method, 'std']:.4f}\n\n")
            ...
            f.write(f"   Average F1: {model_avg.loc[best_model, 'mean']:.4f} ± {model_avg.loc[best_model, 'std']:.4f}\n\n")
...
Examples:
  ...
  python run_all_optimizations.py --n_trials 10 --no_evaluate_test
    ...
    parser.add_argument('--methods', nargs='+', 
                        choices=['bayesian', 'hyperband', 'cmaes'],
                        default=['bayesian', 'hyperband', 'cmaes'],
